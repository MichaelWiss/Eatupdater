...
(function() {
  ...
  FoundationConfirmBox = function(message, element, callback) {
    var $dialog;
    $dialog = $('<div class="reveal-modal small" id="confirmation_dialog" data-reveal> <h4 class="title"></h4> <p class="body"></p> <div class="footer"></div> <a class="close-reveal-modal">&#215;</a> </div>');
    return $dialog.find(".title").html(element.data("confirm-title") || $.fn.foundation_confirmbox.defaults.title || window.top.location.origin).end().find(".body").html(message.replace(/\n/g, "<br />")).end().find(".footer").append($("<a />", {
      href: "#"
    }).html(element.data("confirm-cancel") || $.fn.foundation_confirmbox.defaults.cancel).addClass($.fn.foundation_confirmbox.defaults.cancel_class).addClass(element.data("confirm-cancel-class")).click(function(event) {
      event.preventDefault();
      $dialog.foundation('reveal', 'close').remove();
      return false;
    }), $("<a />", {
      href: "#"
    }).html(element.data("confirm-proceed") || $.fn.foundation_confirmbox.defaults.proceed).addClass($.fn.foundation_confirmbox.defaults.proceed_class).addClass(element.data("confirm-proceed-class") || "btn-primary").click(function(event) {
      event.preventDefault();
      $dialog.foundation('reveal', 'close');
      return callback();
    })).end().appendTo(document.body).foundation('reveal', 'open');
  };
  ...
  $.rails.allowAction = function(element) {
    ...
    if ($.rails.fire(element, "confirm")) {
      FoundationConfirmBox(message, element, function() {
        var allowAction, evt;
